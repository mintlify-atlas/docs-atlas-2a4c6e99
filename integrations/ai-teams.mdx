---
title: AI Team Integrations
description: The service layer for AI development infrastructure - access GPU clusters, model APIs, and training services
---
...
<Steps>
  <Step title="Expose services on your GPU cluster">
    ```bash
  ...
  <Step title="Expose inference services">
    ```bash
...
<Steps>
  <Step title="Morning: connect to your GPU cluster">
    ```bash
    connect reach jupyter-gpu
    ```
  ...
  <Step title="Afternoon: check progress from your phone">
  ...
  <Step title="Evening: training done, check MLflow from home">
    ```bash
    connect reach mlflow
  ...
  <Step title="Deploy: push to inference">
    ```bash
    connect reach model-api
...
<Steps>
  ...
  <Step title="Expose your first service">
    ```bash
    connect expose localhost:8888 --name jupyter-gpu
    ```
  </Step>
  
  <Step title="Reach it from anywhere">
    ```bash
    connect reach jupyter-gpu
    ```
  </Step>
